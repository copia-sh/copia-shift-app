...
管理者としてログインし、ヘッダーの「**設定**」を開いて「**リンクをコピー**」を押します。
...
管理者はヘッダーの「メンバー」から、各メンバーに複数の属性タグを設定できます。
既存のタグはチップから選択し、新しいタグだけ入力欄から追加します
（例: `1班`、`2026夏インターン`）。

属性が1件以上設定されると、全メンバーのカレンダー上部に表示フィルターが現れます。
複数のタグを選んだ場合は、いずれかのタグを持つメンバーを表示します（OR条件）。
「全員」を押すと絞り込みを解除できます。
...
groups/{groupId}/members/{uid}
  email, displayName, color, role, active, attributes[], joinedAt
